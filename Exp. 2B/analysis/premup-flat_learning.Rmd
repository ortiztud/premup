...
```{r}
# Poor performers
poor_performers <- agg_data$participant[agg_data$mean_acc < .4 & agg_data$scn_condition==2]
poor_performers <- c(poor_performers, 7) # Add subject seven due to a different number of trials
...
agg_data$scn_condition <- as.factor(agg_data$scn_condition)
ggplot(data = agg_data,
       mapping = aes(x=scn_condition, y = mean_acc)) +
  geom_boxplot() +
  geom_jitter(shape=10, position=position_jitter(0.2)) +
  ylab("Mean acc") + 
  ylim(c(0,1)) +
...
ggplot(data = agg_data,
  ...
  xlab("Trial N") +
  labs(title = paste("Cummulative acc by prior strength")) +
  theme(legend.position = "top") +
  scale_color_discrete(name = "Prior strength", labels= c("Flat", "Strong")) +
