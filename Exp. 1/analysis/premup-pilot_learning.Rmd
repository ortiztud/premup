...
data <- subset(x = temp,
               select = c("participant", "trial_index", "trial_acc", "scn_condition", "scn_cat", "PE_level"))
...
```{r}
# Poor performers
poor_performers <- agg_data$participant[agg_data$mean_acc < .3]

# Display performance
agg_data %>% filter(participant %in% poor_performers)
...
ggplot(data = agg_data,
  ...
  ylab("Mean acc") + 
  xlab("Prior strength") +
  labs(title = paste("Prediction acc by prior strength")) +
  theme(legend.position = "top") +
  scale_color_discrete(name = "Prior strength", labels= c("Flat", "Weak", "Strong"))

t.test(agg_data$mean_acc[agg_data$scn_condition==1],agg_data$mean_acc[agg_data$scn_condition==2], paired = T, alternative = "less")
```
...
data$trial_n = rep(c(1:n_trials), n_subs)
...
ggplot(data = agg_data,
  ...
  xlab("Trial N") +
  ylim(NA,.75) +
  labs(title = paste("Cummulative acc by scene condition")) +
  theme(legend.position = "top") +
...
First I will look at average acc across scene instances on every trial
```{r}
...
ggplot(data = agg_data,
  ...
  labs(title = paste("Likelihood of choosing the correct object category per trial")) +
   geom_hline(data = agg_data %>% filter(scn_condition==2), aes(yintercept = .8), linetype = "dotted")+
  geom_hline(data = agg_data %>% filter(scn_condition==1), aes(yintercept = .3), linetype = "dotted")+
   theme(legend.position = "top") +
  scale_color_discrete(name = "Prior strength", labels= c("Flat", "Strong")) +
