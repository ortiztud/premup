---
title: "Analysis of day 2 session (refresh + encoding) on PREMUP-three."
...
data <- subset(x = temp,
               select = c("participant", "session", "refr_phase","enc_phase", "mem_phase", "day2_trialN", "trial_acc", "scn_condition", "scn_cat", "PE_level"))

# We'll keep only trials for refresh and encoding (old and filler trials)
data <- data %>% 
  filter(data$refr_phase==1 | data$enc_phase==1)
...
# Compute cummulative acc by participant and prior strength condition. This next bit adds one column to the data frame
data <- data %>% 
...
data2 <- data %>% 
  ...
  mutate(trial_n = c(1:70))
data3 <- data %>% 
  group_by(participant, scn_cat) %>%
  filter(scn_condition == 2) %>% 
  mutate(trial_n = c(1:75))
data4 <- data %>% 
  group_by(participant, scn_cat) %>%
  filter(scn_condition == 3) %>% 
  mutate(trial_n = c(1:140))
...
ggplot(data = agg_data,
  ...
  geom_text(aes(x=40, label="encoding starts", y=.2), colour="black", angle=90, vjust = -1, size=5)+ 
